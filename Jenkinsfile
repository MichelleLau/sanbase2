  ...
  node('sanbase-builder') {
    stage('Run Tests') {
      container('docker') {
        ...
        sh "docker run --rm --name test_clickhouse_${scmVars.GIT_COMMIT} -d yandex/clickhouse-server"
        try {
          sh "docker run --rm
            --link test_postgres_${scmVars.GIT_COMMIT}:test_db
            --link test_influxdb_${scmVars.GIT_COMMIT}:test_influxdb
            --link test_clickhouse_${scmVars.GIT_COMMIT}:test_clickhouse
            --env DATABASE_URL=postgres://postgres:password@test_db:5432/postgres
            --env INFLUXDB_HOST=test_influxdb
            --env CLICKHOUSE_DATABASE_URL=ecto://default@test_clickhouse:8123/default
